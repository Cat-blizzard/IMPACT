...
> **2026-09-17 二次审查修复：** P4 统一终止预算；阶段 A 改为核对真实授权、命令和最终 MAVROS 输出，缺少撤销/过期执行证据保持待验证。尚未重飞，详见 [二次修复记录](docs/CODE_REVIEW_FOLLOWUP_20260917.md)。

> **2026-09-17 代码修复：** 已修复 P4/P16 控制与验收缺口并补回归测试；尚未重飞。地图证据齐全只标记为待内容审查，不自动视为阶段 A 通过。详见 [修复记录](docs/CODE_REVIEW_FIXES_20260917.md)。
