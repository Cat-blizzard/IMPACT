...
def main():
    ...
    results=[]
    driver.create_subscription(PoseStamped,"/uav1/mavros/setpoint_position/local",lambda m:results.append(m.pose.position.x),20)
    ...
    def spin(duration, authorized=False, traj=7, frame="xq_lio_map", session="contract-test"):
        ...
        while time.monotonic()<deadline:
            ...
            c.position.x=.2; c.position.z=2.; command.publish(c)
    ...
    try:
        spin(1.0)
        assert results and max(results)<.01, "uncertified command leaked"
        results.clear(); spin(.6,True)
        assert any(x>.19 for x in results), "certified command did not traverse real DDS/TF"
        results.clear(); spin(.6,True,traj=8)
        assert all(x<.01 for x in results[3:]), "wrong trajectory ID leaked"
        results.clear(); spin(.6,True,frame="world")
        assert all(x<.01 for x in results[3:]), "wrong frame leaked"
        results.clear(); spin(.6,False)
        assert all(x<.01 for x in results[3:]), "revoked trajectory leaked"
        results.clear(); spin(.6,True,session="old-session")
        assert all(x<.01 for x in results[3:]), "old session leaked"
