...
**开发顺序已调整，详见 [开发与验收路线](DEVELOPMENT_ROUTE.md)：先排查共享地图，再做基础目标导航、四策略核心机制实验，最后明确回接自主探索。以下 validate-server/batch 命令描述的是现有旧 P5 门禁实现，新目标导航门禁尚未实现；不得将单目标通过标记为 P5 或自主探索通过。**
...
现有命令仍有“原 P5 先通过”的依赖，代码尚未按新路线切换。旧 P5 失败证据保留；不再要求无限期修复旧探索行为，但共享定位/地图、实际执行和终止检查不能绕过。新协议实现并验证之前，不手写 server-validation.json，也不直接放行正式矩阵。接下来的开发按 DEVELOPMENT_ROUTE.md 和 SERVER_HANDOFF.md 执行。
...
- 共享地图排查；独立命名的基础目标导航与失败终止验收。旧 P5 完整复现状态单独保留。
...
- 按 C1—C4 回接自主目标生成、完整性参与目标选择、恢复后续接和探索结束判定，开展独立探索级实验。
